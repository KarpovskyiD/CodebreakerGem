...
RSpec.describe Codebreaker::Game do
  context 'Checking different values initialization if new game was created' do
    ...
    context 'Checking functionality of attempts assigning according to level' do
      it 'level assigned easy' do
        expect(Codebreaker::Game.new(:easy).attempts_total). to eq(Codebreaker::Game::DIFFICULTIES.dig(:easy, :attempts))
      end

      it 'level assigned medium' do
        expect(Codebreaker::Game.new(:medium).attempts_total). to eq(Codebreaker::Game::DIFFICULTIES.dig(:medium, :attempts))
      end

      it 'level assigned hard' do
        expect(Codebreaker::Game.new(:hard).attempts_total). to eq(Codebreaker::Game::DIFFICULTIES.dig(:hard, :attempts))
      end
    end
